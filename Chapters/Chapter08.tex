...
\medskip Der Benutzer soll das Programm gar nicht bemerken, muss aber dennoch ein gewisses Maß an Feedback erhalten, um sinnvoll mit \scribbler arbeiten zu können. Dass der Zeichenmodus aktiv ist, wird dem Benutzer beispielsweise dadurch mitgeteilt, dass statt dem normalen Pfeil-Cursor ein Fadenkreuz-Cursor angezeigt wird. Zusätzlich markiert \scribbler das aktive Fenster (einer Drittapplikation) mit einem blauen Rahmen. Dem Benutzer wird so kommuniziert, welchem Fenster seine Zeichnungen zugeordnet werden. Er kann sich also vergewissern, ob er tatsächlich auf das gewünschte Fenster zeichnet.

\scribbler hat ein sehr subtiles \ac{GUI}, das sich möglichst im Hintergrund hält, um den Benutzer nicht von seinem eigentlichen Vorhaben abzulenken. Bei normaler Interaktion mit Maus und Tastatur durch den Benutzer, zeigt sich \scribbler ausschließlich in der Systemleiste am oberen Rand des Bildschirms als sternförmiges Icon, siehe \autoref{fig:scribblerIcon}. 
\begin{figure}
	\begin{center}
        \myfloatalign
